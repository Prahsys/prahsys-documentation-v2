...
<Callout theme="default">
  **Payment ID**s are created/generated by you before making API calls. These IDs should be unique identifiers
  in your system and are passed in the request URLs. You can use your own Payment ID or create one with the [Update or Create Payment](/reference/updateorcreatepayment) endpoint.
</Callout>
...
To create a Pay Portal session, send a POST request to the [Create Session](/reference/createsession) endpoint and include the `"portal"` object in the request:

> If the `"portal"` object is not included in the request, then a **pay session** will be created.
>
> Allowed operations are: `PAY`, `AUTHORIZE`, `VERIFY`, or `NONE`. These map to the [Pay](/reference/pay), [Authorize](/reference/authorize), and [Verify](/reference/verify) transaction operations.
>
...
The [Create Session](/reference/createsession) response contains the session ID and `successIndicator`.
...
When the payment is complete, the customer will be redirected to the `returnUrl` specified in your [Create Session](/reference/createsession) request. The session response from Step 1 includes a `successIndicator` value that you can use to verify the payment status.

#### Using the successIndicator

The `successIndicator` is a unique identifier returned in the [Create Session](/reference/createsession) response that helps you verify the payment result.
...
You should also verify the session or payment status with the [Get Session](/reference/getsession) or [Get Payment](/reference/getpayment) endpoint before fulfilling orders.

## Best Practices

* Always create Pay Portal sessions server-side with the [Create Session](/reference/createsession) endpoint, never client-side
...
* Verify the payment status with the [Get Session](/reference/getsession) or [Get Payment](/reference/getpayment) endpoint before fulfilling orders
