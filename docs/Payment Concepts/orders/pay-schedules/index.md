...
| `billing`               | object  | `null`              | Saved payment method. The `billing` field is optional when creating the order, but it must be present before [starting a schedule](#starting-a-pay-schedule) with autopay enabled. See [setting up autopay](#setting-up-autopay) section to learn how to attach a billing method. |
